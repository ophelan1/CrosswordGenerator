...
void printClues(void){
	...
	int strLen;
	int intersection;	//boolean value
	...
	for( i = 0 ; i < 15 ; i++){
		for( j = 0 ; j < 15 ; j++){	
			...
			if ( ((isalpha(board[i][j-1]) == 0) || (i == 0)) && (isalpha(board[i][j]) != 0) && (isalpha(board[i][j+1]) != 0) ) {
				if ( ((isalpha(board[i-1][j]) == 0) || (j == 0)) && (isalpha(board[i][j]) != 0) && (isalpha(board[i+1][j]) != 0) ) {
					ptrcmp = &board[i][j];
					k = 0, down = 0;
					while(*ptrcmp != '-') {
						cmpStr[k] = *ptrcmp;
			...
			else if ( ( ((isalpha(board[i-1][j]) == 0) || (j == 0)) && (isalpha(board[i][j]) != 0) && (isalpha(board[i+1][j]) != 0) ) || (intersection == 1)) {
